...
let testApples = apples + oranges ;
let testApplesStr = "I have \(testApples) pieces of fruit";

//1_8 数组也是基本类型,不再是OC中引用类型
var shoppingList = ["catfish","water","tulips","blue paint"];
shoppingList[1] = "bottle of water";
print("YYLog:", shoppingList)
//1_9 如何定义一个空数组
...
for (_,value) in interestingNumbers {
    for number in value {
        largest = number;
//        print(largest); [sd, sdfsdf, henanshengbagongqiaozhenyujiawacun]
    }
}
//print("YYLog:", largest)
...
//hasAnyMatches(list: [], condition: lessThanTen(number: 9)) //为什么会报错
...
class EquilateralTriangle : NamedShape {
    ...
    //这个是setter 和 getter方法的定义   什么时候调用的？
    var perimeter : Double {
        get{
            print("YYHLog","getter 调用了")
            return 3.0 * sideLength;
        }
        set{
            print("YYHLog","setter 调用了")
...
class TriangleAndSquare {
    ...
    var triangle: EquilateralTriangle {
        willSet{
            print("YYHLog","triangle willSet 调用了")
    ...
    var square: Square {
        willSet{
            print("YYHLog","square willSet 调用了")
...
protocol ExampleProtocol {
    ...
    mutating func adjust()  // mutating 神马意思？
}
